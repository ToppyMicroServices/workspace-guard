...
Prevent accidental home-directory opens in VS Code and review risky repository automation before trusting a repository.
...
5. Open the Problems view to inspect restricted-mode diagnostics for risky repository config such as `tasks.json`, `launch.json`, `mcp.json`, `settings.json`, and `*.code-workspace`.
6. Run `Workspace Guard: Review .github Automation` from the Command Palette if you want the same review in the output panel.
7. If you want to inspect a repository from the terminal, run `workspace-guard-scan` in that repository.

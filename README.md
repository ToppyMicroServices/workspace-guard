...
- `.github/workflows/*.yml`: `docker://` actions without digest pin, external reusable workflows, `secrets: inherit`, scheduled/manual triggers, tainted `env` values consumed by `run`, and untrusted GitHub-context interpolation into shell execution or execution-sensitive `with:` inputs
